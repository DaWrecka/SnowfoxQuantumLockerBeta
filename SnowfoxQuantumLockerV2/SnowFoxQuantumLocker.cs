...
namespace SnowFoxQuantumLocker
{
    internal class SnowFoxQuantumLocker
    {
        ...
        internal class PatchHoverbikeUpdate
        {
            ...
            {
                if (__instance.GetPilotingCraft() && GameInput.GetKeyDown(QMod.Config.LockerKey))//GetPilotingCraft() just returns true if the player is currently riding the snowfox
                {
        ...
        [HarmonyPatch(typeof(Hoverbike))]
        internal class HoverbikeStartPatch
        {
            [HarmonyPatch(nameof(Hoverbike.Start))]
            [HarmonyPostfix]
            internal static void PostStart(Hoverbike __instance)
            {
                if (QMod.Config.LockerType.Equals("Quantum"))
                {
                    ...
                    __instance.gameObject.EnsureComponent<VehicleQuantumLockerComponent>();
                }
            }
        }

        public class VehicleQuantumLockerComponent : QuantumLocker
        {
            public void Awake()
            {
                CoroutineHost.StartCoroutine(DeferredInit());
            }
        ...
        class SnowfoxQuantumStorage : /*MonoBehaviour,*/ QuantumLocker, IProtoEventListener
        {
